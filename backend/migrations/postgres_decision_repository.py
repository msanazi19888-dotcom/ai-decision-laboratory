...
class PostgreSQLDecisionRepository(DecisionRepository):
    def save(self, decision: Decision) -> Decision:
        with SessionLocal() as session:
            # Remove old rows if the decision already exists
    ...
    def _to_model(self, decision: Decision) -> DecisionModel:
        model = DecisionModel(
            ...
            selected_strategy_id=(
                decision.selected_strategy.id if decision.selected_strategy else None
            ),
        ...
        model.strategies = [
            self._strategy_to_model(decision.id, strategy, decision.selected_strategy)
